...
function display_tasks(filter = "all") {
  ...
  if (!tasksListElement) return; // Exit if the tasks list element is not found
  ...
  //updating counts(independent of filter)
  document.getElementById("pending_count").textContent = taskList.filter(
    (tasks) => !tasks.completed,
  ).length;

  // if (!tasksListElement) return; // Exit if the tasks list element is not found

  document.getElementById("completed_count").textContent = taskList.filter(
    (tasks) => tasks.completed,
  ).length;
  // if (!tasksListElement) return; // Exit if the tasks list element is not found
...
function load_tasks_locally() {
  ...
  if (storedTasks && document.getElementById("tasks_list")) {
